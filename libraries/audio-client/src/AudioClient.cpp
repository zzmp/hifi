...
AudioClient::AudioClient() :
    ...
    connect(&_inputs, &AudioInputs::deviceListLoudnessChanged, this, &AudioClient::inputListLoudnessChanged);
...
#ifdef Q_OS_WIN
QString AudioClient::getWinDeviceName(IMMDevice* pEndpoint) {
    QString deviceName;
...
QAudioDeviceInfo defaultAudioDeviceForMode(QAudio::Mode mode) {
#ifdef __APPLE__
    if (AudioClient::getAvailableDevices(mode).size() > 1) {
    ...
    if (!IsWindowsVistaOrGreater()) { // lower then vista
        ...
        if (hr == E_NOTFOUND) {
            ...
            deviceName = AudioClient::getWinDeviceName(pEndpoint);
        }
        pMMDeviceEnumerator->Release();
